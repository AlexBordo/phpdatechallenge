...
class MyDateTest extends TestCase
{
    ...
    public function exceptionFixtures()
    {

        $exception = \InvalidArgumentException::class;

        return [
            ['202903/16', '2013/06/18', $exception],
            ['2003-03-16', '2010/03/16', $exception],
            ['2029/03/16', '2029a/03/9', $exception],
            ['2010/03/16', '2010/50/16', $exception],
            ['2010/03/16', '2010/10/16/123a', $exception],
            ['', '', $exception]
        ];
    }
     ...
     * @dataProvider exceptionFixtures
     */
    public function testMyDateParserArgumentException($a, $b, $expected)
    {

        $this->assertInstanceOf($expected, \AB\MyDate\MyDate::diff($a, $b));
    ...
    private function assertTotalDays($s, $e)
    {
        $d = MyDate::diff($s, $e);
        $a = $this->dateDiff($s, $e);
        $this->assertSame(intval($a->days), intval($d->total_days));
    }
